...
document.addEventListener('DOMContentLoaded', async ()=>{
  ...
  try{
    ...
    // favoritos
    const favsDiv = document.getElementById('favoritos');
    ...
    favs.forEach(id=>{
      ...
      const div = document.createElement('div');
      div.className='card';
      div.innerHTML=`<img loading="lazy" src="${p.imagen}" alt="${p.nombre}"><h4>${p.nombre}</h4><p><strong>${p.precio>0?('$'+p.precio):'Pedido'}</strong></p>`;
      favsDiv.appendChild(div);
    });
    // Add event delegation for Ver buttons to open modal via catalogo module
    ...
    const grid = document.getElementById('destacados-grid');
    grid.innerHTML='<p style="color:var(--muted)">No se pudieron cargar los productos.</p>';
  }
});
